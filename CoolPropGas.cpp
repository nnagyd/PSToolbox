...
double CoolPropGas::Get_kappa(double T, double p) {
	double cp = Get_cp(T,p); 
	double cV = Get_cV(T,p);
	return (cp/cV);
}
...
double CoolPropGas::Get_Rspec(){
	double R=PropsSI("GAS_CONSTANT","P",1e5,"T",300,fluid_name);
	double M=PropsSI("MOLAR_MASS","P",1e5,"T",300,fluid_name);
	return (R/M); 
}
