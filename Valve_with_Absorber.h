...
/*! \brief A class for handling both liquid and fluid valves
	...
	*/
class Valve_with_Absorber: public Units
{
	private:
		...
		double x; /*!< displacement of the valve during the simulation */
		double v; /*!< velocity of the valve head during the simulation*/
		double p; /*inlet side pressure in the valve */
		...
		double t_monitor_start;
		double x_simul_min; /*! minimum x value reached during simulation*/
		double x_simul_max; /*!<maximum x value reached during simulation */
		...
		double ma,ka,sa; // Absorber mass, damping and spring stiffness
		double xa,va; // Absorber location and velocity
	...
	public:
		Valve_with_Absorber(const string _nev,
				const double _m,
				const double _k,
				const double _s,
				const double _Dbore,
		...
		double SumStatForce(const double x, const double pv, const double pb, const double xa);
};
